...
def global_CEI(opt_index, cond_mean, cond_var, cond_cov, x_opt_cond_mean, x_opt_cond_var):
...
# print(cei_value)
    ...
    # V(x, x~)
    var_of_dif = cond_var + x_opt_cond_var # under this case C(x, x~) = 0
    ...
    # CEI
    cei = dif_of_mean * norm.cdf(crit_val) + np.sqrt(abs(var_of_dif)) * norm.pdf(crit_val)
    if sum(cei) == 0:
