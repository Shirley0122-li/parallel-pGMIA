...
from likelihood import likelihood_new_sol
...
#########################################
    ...
    # bounds tau[0] > 1e-6
    temp = 1/np.var(all_reg_output[sampled_reg_index])
    reg_bnds = [(temp / 100, 10 * temp)]    # tau[0]
    ...
    try:
        ...
        # in case the optimizie.minimize failed to find a solution because of the constraint
        # print('Tried, but failed')
        n_theta_to_test = 1
        theta_to_test = [0.45/dim_sol for i in range(dim_sol)]
        theta_sol0 = [1e-8]
        theta_sol = list(theta_sol0) + list(theta_to_test)
