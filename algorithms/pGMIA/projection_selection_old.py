...
def projection_selection(n_init_reg, n_init_sol, n_rep, grid_dim, dim_reg, obj_func, sampled_vec, sampled_rep, sampled_mean, sampled_var):
...
# projection_selection(n_init_reg, n_init_sol, n_rep, grid_dim, dim_reg, obj_func, sampled_vec, sampled_rep, sampled_mean, sampled_var)
    ...
    if len(sampled_reg_index) > 100:
        selected_sampled_reg_index = random.sample(sampled_reg_index, 100)
    else:
    ...
    tau = res.x       # array
    beta = likelihood_fun(tau, grid_dim_reg, design_reg_index, all_reg_output[design_reg_index], np.diag(all_reg_var[design_reg_index]), 0, 0)[1]
    ...
    print('Solution-layer MLE time cost', time_end-time_start, 's')
