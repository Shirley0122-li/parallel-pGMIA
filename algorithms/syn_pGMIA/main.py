...
def pGMIA(file_name, NAI, max_iter, size, master_generator, period):
...
# max_iter (scalar): simulation budget 
    ...
    # period = 50    # period for pGMIA to update the projection
    ...
    for iter in range(max_iter):
        ...
        for i in range(size - 1):
            ...
            data = [reg, n_rep, n_init_sol, theta_sol, grid_dim_reg, grid_dim_sol, shift_vector, obj_func, \
                    all_sol_rep[reg], all_sol_output[reg], all_sol_var[reg], master_start]
            req = comm.isend(data, dest = i + 1, tag = i)
            req.wait()
            # master_end = time.time()
            # print('send data to worker', i + 1, 'time:', master_end - master_start)

        # receive data from workers
        for i in range(len(reg_to_simulate)):
            reg = reg_to_simulate[i]
            # master_start = time.time()
            req = comm.irecv(source = i + 1, tag = i)
            output = req.wait()
            # master_end = time.time()
            # print('receive data for region', reg, 'time:', master_end - master_start)
            ...
            # if this region is the region containing optimal solution
            if reg == opt_sol_reg:
                cur_total_opt_val = np.concatenate((cur_total_opt_val, output[-4]))    # add 20 iterations of opt val
                cur_total_max_CEI = np.concatenate((cur_total_max_CEI, output[-3]))    # add 20 iterations of max CEI
                # print('cur_total_opt_val', cur_total_opt_val)

            # update the dataset 
            all_sol_rep[reg], all_sol_output[reg], all_sol_var[reg], cur_true_opt, max_CEI_val, reg, time_temp = output
            # find the sampled sol in this region 
    ...
    #########################################################################################################
        ...
        if iter % period == period - 1:
            ...
            # # close the file
            # file.close()
            # print('current time:', time.time()-start_time, 'at iter', iter)
    ...
    return [cur_opt_sol, cur_opt_true_val, cur_opt_val, cur_num_sampled, cur_num_reps, \
        cur_opt_reg, cur_opt_sol_reg, cur_max_CEI_reg, cur_reg_CEI_max_val, cur_total_opt_val, \
            cur_sampled_in_opt_reg, cur_total_max_CEI, cur_run_time, time_record]
...
# print(sys.argv)
seed = int(sys.argv[1])
reg_period = int(sys.argv[2])
sol_max_iter = int(sys.argv[3])
file_name = 'sync' + str(size) + 'c' + str(seed) \
    + 'all'
...
if rank == 0:
    ...
    output = pGMIA(file_name, seed, max_iter, size, processor_generator, reg_period)
    ...
    for iter in range(max_iter):
        ...
        # print('worker', rank, 'receive data from master', 'time:', worker_end - worker_start)

        # record the data from the master
        [this_reg, n_rep, n_init_sol, theta_sol, grid_dim_reg, grid_dim_sol, shift_vector, obj_func, \
            sol_rep, sol_output, sol_var, master_start] = data
        
        # for each region, run solution-layer GMIA
        
        output = FineLevel_GMRF(this_reg, n_rep, n_init_sol, theta_sol, grid_dim_reg, grid_dim_sol, shift_vector, obj_func, \
                sol_rep, sol_output, sol_var, sol_max_iter, processor_generator)
        
        # send data to the master
        # worker_start = time.time()
        output.append(this_reg)
        output.append([master_start, worker_start])
        req = comm.isend(output, dest=0, tag=i)
        req.wait()
        worker_end = time.time()
        # print('worker', rank, 'solution layer ', 'time:', worker_end - worker_start)
        # print('worker', rank, 'send data to master', 'time:', worker_end - worker_start)

    # mpiexec -n 8 python main.py
    # mpiexec -n 4 python -u main.py 1 3 10
    # the first input is seed, 
    # the second is region-layer period, 
    # the third is number of max solution-layer iterations 