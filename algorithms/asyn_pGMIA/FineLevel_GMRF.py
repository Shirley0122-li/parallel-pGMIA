...
# Sampling criterion
from CEI import *
from global_EI import *
from relation_of_sol_and_reg import check_if_in_reg
from relation_of_sol_and_reg import check_if_neighbor
from update_prec_mat import *
from global_CEI import *

def FineLevel_GMRF(this_reg, n_rep, n_init_sol, theta_sol, grid_dim_reg, grid_dim_sol, shift_vector, obj_func, \
    sol_rep, sol_output, sol_var, max_iter, delta, count, generator, 
    grid_dim, theta, x_opt, x_opt_output, x_opt_output_var, x_opt_cond_mean, x_opt_cond_var):
    ...
    max_iter (scalar): number of iterations of solution-layer GMIA to update
    delta (scalar): threshold of solution-layer CEI value (stopping criterion)
    x_opt (1d array, size = dim): global optimal solution
    x_opt_cond_mean (scalar): conditional mean at global optimal solution
    x_opt_cond_var (scalar): conditional variance at global optimal solution
    ...
    max_iter = 20
    delta = 0
    FineLevel_GMRF(this_reg, n_rep, n_init_sol, theta_sol, grid_dim_reg, grid_dim_sol, shift_vector, \
        obj_func, sol_rep, sol_output, sol_var, max_iter, delta)
    """
    
    # sampling criterion
    sample_criterion = CEI
    ...
    cur_posterior_mean = np.empty(max_iter)
    cur_posterior_var = np.empty(max_iter)
    reg_vec = np.unravel_index(this_reg, grid_dim_reg)     # tuple of reg_dim integers

    ## temp data structure for recording
    cur_cei_sol = np.empty(max_iter)
    cur_pos_mean_sol = np.empty(max_iter)
    cur_pos_var_sol = np.empty(max_iter)
    ...
    theta_sol = np.array([1e-8, 0.15, 0.15, 0.15])
    ...
    if n2 < n_init_sol:
        ...
        for j in range(n_init_sol):   
            ...
            if design_sol_index not in set(sampled_sol_index):
                ...
                sim_output = obj_func(orig_design_sol, n_rep, generator)   # [1, n_rep] ndarray, 
    ...
    # print('theta_sol', theta_sol, 'beta', beta)
    ...
    for iter in range(max_iter):
        ...
        # compute solution-layer CEI/global EI/global CEI
        # there are three cases under global CEI
        if sample_criterion == global_CEI:
            # case 1: global optimum is in the region
            if len(x_opt) == 0 or check_if_in_reg(x_opt, this_reg, shift_vector, dim_reg, grid_dim_reg):
                cei_val = CEI(opt_sol, cond_mean, cond_var, cond_cov, x_opt_cond_mean, x_opt_cond_var)

            # case 2: global optimum is in the neighbor of region
            elif check_if_neighbor(x_opt, this_reg, shift_vector, dim_reg, grid_dim_reg):
                ## recompute the precision matrix, cond_mean, con_var and cond_cov
                new_prec_mat = update_prec_mat(Q, theta, x_opt, this_reg, shift_vector, dim_reg, grid_dim)
                # Get the shape of the existing array
                rows, columns = Q_epsi.shape
                # Create a new array with an additional row and column of zeros
                new_Q_epsi = np.zeros((rows + 1, columns + 1))
                # Copy the existing array into the new array
                new_Q_epsi[:-1, :-1] = Q_epsi
                new_Q_epsi[-1,-1] = 1/x_opt_output_var

                # add global optimum to data
                new_sol_output = np.append(sol_output, x_opt_output)
                new_opt_sol = rows
                new_sampled_sol_index = np.append(sampled_sol_index, rows)
                new_Q = new_prec_mat + new_Q_epsi
                new_Q_inv = np.linalg.inv(new_Q)

                # conditional means
                cum_change = np.dot(new_Q_epsi, new_sol_output - beta)
                cond_mean = beta + np.dot(new_Q_inv[:, new_sampled_sol_index], cum_change[new_sampled_sol_index])
                
                # conditional variance and covariance
                cond_var = np.diag(new_Q_inv)    # 1d array of size(n)
                cond_cov = new_Q_inv[:, new_opt_sol].flatten()    # 1d array of size(n)
                cei_val = CEI(new_opt_sol, cond_mean, cond_var, cond_cov, x_opt_cond_mean, x_opt_cond_var)

            # case 3: global optimum is not in the region nor in the neighborhood
            else:
                cei_val = sample_criterion(opt_sol, cond_mean, cond_var, cond_cov, x_opt_output, x_opt_output_var)
        
        # CEI or global EI
        else:
            cei_val = sample_criterion(opt_sol, cond_mean, cond_var, cond_cov, x_opt_cond_mean, x_opt_cond_var)

        # local_cei_val = CEI(opt_sol, cond_mean, cond_var, cond_cov, x_opt_cond_mean, x_opt_cond_var)
        # global_ei_val = global_EI(opt_sol, cond_mean, cond_var, cond_cov, x_opt_output, x_opt_cond_var)
        ...
        cur_posterior_mean[iter] = cond_mean[cei_sol]
        cur_posterior_var[iter] = cond_var[cei_sol]

        # temp recording
        cur_cei_sol[iter] = cei_sol
        temp = np.where(cond_mean == np.min(cond_mean))
        cur_pos_mean_sol[iter] = np.take(temp, 0)
        temp = np.where(cond_var == np.max(cond_var))
        cur_pos_var_sol[iter] = np.take(temp, 0)

        # if max_CEI_val < threshold, the solution-layer GMIA is stopped immediately
        #  if max_CEI_val < delta:
        #     break
        ...
        for sol in (opt_sol, cei_sol):
            ...
            # record the optimal solution vector
            if sol == opt_sol:
                opt_sol_sub = orig_design_sol[0]
    ...
    # filename = "region" + str(this_reg) + "count" + str(count)
    # file = open(filename, 'wb')
    # # dump information to that file
    # output = [sol_rep, sol_output, sol_var, cur_opt_true_val, cur_max_CEI, \
    #     cur_posterior_mean, cur_posterior_var, cur_cei_sol, cur_pos_mean_sol, cur_pos_var_sol, max_CEI_val,\
    #         cei_val, local_cei_val, global_ei_val]
    ...
    return [sol_rep, sol_output, sol_var, cur_opt_true_val, cur_max_CEI, \
        ...
        opt_sol_sub, sol_output[opt_sol], sol_var[opt_sol], cond_mean[opt_sol], cond_var[opt_sol],\
        max_CEI_val, count+1]
